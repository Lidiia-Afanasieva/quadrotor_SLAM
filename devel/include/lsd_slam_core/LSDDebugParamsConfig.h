...
namespace lsd_slam_core
{
  ...
  public:
    ...
    public:
      ...
      virtual void clamp(LSDDebugParamsConfig &config, const LSDDebugParamsConfig &max, const LSDDebugParamsConfig &min) const override
      {
      ...
      virtual void calcLevel(uint32_t &comb_level, const LSDDebugParamsConfig &config1, const LSDDebugParamsConfig &config2) const override
      {
        if (config1.*field != config2.*field)
          comb_level |= level;
      }

      virtual void fromServer(const ros::NodeHandle &nh, LSDDebugParamsConfig &config) const override
      {
        nh.getParam(name, config.*field);
      }

      virtual void toServer(const ros::NodeHandle &nh, const LSDDebugParamsConfig &config) const override
      {
        nh.setParam(name, config.*field);
      }

      virtual bool fromMessage(const dynamic_reconfigure::Config &msg, LSDDebugParamsConfig &config) const override
      {
        return dynamic_reconfigure::ConfigTools::getParameter(msg, name, config.*field);
      }

      virtual void toMessage(dynamic_reconfigure::Config &msg, const LSDDebugParamsConfig &config) const override
      {
        dynamic_reconfigure::ConfigTools::appendParameter(msg, name, config.*field);
      }

      virtual void getValue(const LSDDebugParamsConfig &config, boost::any &val) const override
      {
        val = config.*field;
      }
    };
    ...
    public:
      ...
      virtual void updateParams(boost::any &cfg, LSDDebugParamsConfig &top) const override
      {
        PT* config = boost::any_cast<PT*>(cfg);
